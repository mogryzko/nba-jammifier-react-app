...
def RANSAC_poly(x, y, max_iter, eps):
    ...
    for i in range(max_iter):
        ...
        if temp_poly.coef[-1] < 0.25: continue
...
def exaggerated_poly(gauss20, exaggeration=50):
    ...
    ys_fit = [y_poly[0], np.min(y_poly) - exaggeration, y_poly[len(gauss20)-1]]
...
def perspective_transform(xys, H):
    return cv2.perspectiveTransform(xys.reshape(-1,1,2), H).reshape(-1,2)
...
def overlay_gif(original_gif, Hs, masks, xs, ys, jump_start_frame_num, jump_end_frame_num, stab_gif, stab_gif_masks):
    ...
    for i in tqdm(range(len(original_gif))):
        if jump_start_frame_num <= i <= jump_end_frame_num:
            ...
            player = cv2.bitwise_and(original_gif[i], original_gif[i], mask=shaped_mask(masks[i - jump_start_frame_num]))
            # remove player from image
            background = cv2.bitwise_and(original_gif[i], original_gif[i], mask=1 - shaped_mask(masks[i - jump_start_frame_num]))

            mask_pts = zip(*np.nonzero(masks[i - jump_start_frame_num]))
            mask_pts = list(mask_pts)
            mask_pts = np.asarray(mask_pts)
            dilated = scipy.ndimage.morphology.binary_dilation(masks[i - jump_start_frame_num], iterations=15)
            ...
            adj_centroid = np.asarray(adj_centroids[i - jump_start_frame_num], dtype=np.float32).reshape(-1, 1, 2) # reshape for perspectiveTransform
            ...
            transformed_centroid = np.array(transformed_centroid, dtype=np.int64)
